...
/**
 * EventActivity display the content information of activity 1.
 *
 * @since 2022-08-02
 * <p>
 * Copyright (c) HuaWei Technologies Co., Ltd. 2012-2022. All rights reserved.
 */
public class EventActivity extends AppCompatActivity implements UserInfoAction.UiCallBack{
    ...
    private void openDB(){
        ...
        mHandler.post(() -> {
            ...
            userInfoAction.addCallBacks(EventActivity.this);
    ...
    private void insertDB(){
        ...
        mHandler.post(() -> {
            userInfoAction.upsertUserInfo(userInfoAction.buildUserInfo(
                    uid, phoneNumber, date, true, finalEvent));
        });
    }
    ...
    /**
     * Backing Up and Querying All Logical Method.
     */
    private void backup(){
        //Query the cloud database and check whether the login user has a cloud database.
        Boolean flag = false;//By default, no identical user exists.
        for (int i = 0; i < list.size(); i++){
            String cuid = list.get(i).getUid();
            if (cuid.equals(uid)){
                flag = true;//The same user exists in the cloud database.
                break;
            }
        }
        if(flag){
